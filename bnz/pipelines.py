...
class BnzPipeline:
    ...
    # Database setup
    conn = sqlite3.connect('bnz.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `bnz`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM bnz WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO bnz (date, title, link, content)"
